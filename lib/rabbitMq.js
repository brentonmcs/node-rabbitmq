...
function RabbitMq(queueUri, queueName, amqp) {
    ...
    addRabbitListener("ConnectedRec", function(queueInfo) {
        ...
        console.log(queueInfo.queueOptions);
        queueInfo.ch.bindQueue(queue, RabbitMq.prototype.queueName(), queueInfo.queueOptions.messageRoute);

        queueInfo.ch.consume(queue, queueInfo.callback, {
            noAck: true
        }, function(err, ok) {
